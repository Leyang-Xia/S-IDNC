...
- **离群**：`OUTLIER_TH=0.5`，`OUTLIER_L=3`，`RESCUE_PERIOD=6`
- **样本**：`n_min=30`
...
const EXPLORE_RATIO=0.10, OUTLIER_TH=0.5, OUTLIER_L=3, RESCUE_PERIOD=6
const τ_bad=2, n_min=30
...
state:
  ...
  m_curr ← default_mcs
  hold_counter ← 0
  outlier_bad_streak[i] ← 0
  rescue_counter ← 0
  bad_streak_counter ← 0  // 跟踪主簇低于阈值的轮数

loop each round:
  // A) 用 ACK 更新统计（EWMA 老化）
  for tx in last_round_tx:
    for user i in users:
      if i was addressed in tx:
        attempt[i][tx.mcs] += 1
        recv ← ack_received(i, tx) ? 1 : 0
        success[i][tx.mcs] += recv
        x ← window_success_rate(i, tx.mcs)         // success/attempt in recent window
        s[i][tx.mcs] ← ALPHA*s[i][tx.mcs] + (1-ALPHA)*x

  // A.1) 用户级单调修正（缺失处理：前缀最高 + PAV 融合）
  for user i in users:
    // 构造前缀最高填充序列 s_pref
    s_pref[*] ← s[i][*]
    max_seen ← -inf
  ...
  // G) 发送计划
  K_total ← plan_tx_count_for_round()
  K_explore ← round(EXPLORE_RATIO * K_total)
  K_main ← K_total - K_explore - (rescue_counter % RESCUE_PERIOD == 0 ? 1 : 0)

  plan ← []
  append K_main of {mcs: m_curr} to plan
  // 探索档位：在 {m_curr+1, m_curr+2, m_curr-1, m_curr-2} 中按 round-robin 轮转，越界或禁用档位跳过
  explore_set ← next_round_robin({m_curr+1, m_curr+2, m_curr-1, m_curr-2})
  distribute K_explore across explore_set into plan
  if rescue_counter % RESCUE_PERIOD == 0: plan.push({mcs: m_safe_low})
  rescue_counter += 1

  execute(plan); collect ACKs
end loop
```
