...
typedef struct {
    int32_t alpha;          /* EWMA 遗忘因子（定点数，FIXED_SCALE=1000 表示 1.0） */
    int32_t zeta;           /* 活跃簇占比阈值（定点数） */
    int32_t beta;           /* 簇权重幂次（定点数） */
    int32_t gamma;          /* 评分幂次（定点数） */
    int32_t deltaUp;        /* 升档收益阈值（定点数） */
    int32_t deltaDown;      /* 降档收益阈值（定点数） */
    int holdTimeUp;         /* 升档防抖轮数 */
    int holdTimeDown;       /* 降档防抖轮数 */
    int32_t noSampleDecay;  /* 未尝试档位的衰减系数（定点数） */
    int nMin;               /* 升档所需最小样本数 */
} MCSConfig;
...
typedef struct {
    ...
    const int32_t *rTable;                         /* 各档位速率表（定点数，FIXED_SCALE 表示 1.0 Mbps） */
    int mCurr;                                     /* 当前使用的 MCS */
    int holdCounterUp;                             /* 升档防抖计数 */
    int holdCounterDown;                           /* 降档防抖计数 */
    int32_t successRate[MCS_MAX_LEVELS * MCS_MAX_USERS]; /* EWMA 成功率（定点数，按 MCS 主序存储） */
...
typedef struct {
    ...
    int32_t weights[MCS_MAX_CLUSTERS];             /* 各簇权重（定点数） */
} MCSClusterSet;
...
typedef struct {
    ...
    int prevMcs;                                   /* 上一轮 MCS */
    int mCurr;                                     /* 本轮 MCS */
    int64_t scoreCurr;                             /* Score(mCurr)（定点数，扩大比例） */
    int64_t scoreUp;                               /* Score(mCurr+1)（定点数） */
    int64_t scoreDown;                             /* Score(mCurr-1)（定点数） */
    int holdCounterUp;                             /* 升档防抖计数 */
    int holdCounterDown;                           /* 降档防抖计数 */
    int clusterCount;                              /* 活跃簇数量 */
    int clusterSizes[MCS_MAX_CLUSTERS];
    int32_t clusterWeights[MCS_MAX_CLUSTERS];      /* 各簇权重（定点数） */
    int32_t clusterSuccess[MCS_MAX_CLUSTERS];      /* 各簇在当前档位的代表成功率（定点数） */
 ...
 *   successRatio: 本轮各用户/MCS的成功率（定点数） (user_major, size = userCount * mcsLevels)
